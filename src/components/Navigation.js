        ...
        <div className="flex">
          {menuLinks.map((link) => (
            <Link
              key={link.name}
              to={link.link}
              className="px-4 py-2 mx-1 font-medium text-gray-600 rounded hover:bg-gray-200 hover:text-gray-900"
              activeClassName="bg-gray-200 text-gray-900 shadow-inner"
            >
              {link.name}
            </Link>
          ))}
